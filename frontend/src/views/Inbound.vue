<template>
  <div class="page">
    <div class="header">
      <h2>入库管理</h2>
      <button @click="showModal = true" class="btn-primary">新增入库</button>
    </div>
    ...
    <div v-if="showModal" class="modal">
      <div class="modal-content">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="form-group">
            <label>来源</label>
            <input v-model="form.source_info" />
          </div>
          <div class="form-group">
            <label>操作人</label>
...
const loadData = async () => {
  const res: any = await api.inbounds.list()
  list.value = res.data
}
...
const closeModal = () => {
  ...
  form.value = { warehouse_id: '', storage_location_id: '', product_id: '', quantity: 0, source_info: '', operator: '' }
  locations.value = []
}

const submitForm = async () => {
  await api.inbounds.create(form.value)
  closeModal()
  loadData()
}
