...
function toBuild() {
    ...
    local build_target_name=${build_path}/${RUN_MODE}/${product_name}/${product_name}
    if [[ "$OS_TYPE" == "Windows" ]]; then
        build_target_name=${build_path}/${RUN_MODE}/${product_name}/${product_name}.exe
    fi
    
    go build -o ${build_target_name} -trimpath -ldflags "${ld_flag_master}" main.go \
    && chmod a+x ${build_target_name} \
    ...
    && cp ./example_files/config_example.yaml ${build_path}/${RUN_MODE}/${product_name}/conf/config.yaml


    for extend_app in "${extend_app_names[@]}"; do
        if [ -z "$extend_app" ]; then
            continue
        fi

        if [ ! -d "./${product_name}_${extend_app}" ]; then
            echo "warning: app dir [./${product_name}_${extend_app}] No such，skeep build"
            continue
        fi
...
function package_files(){
    ...
    && if [[ "$OS_TYPE" == "Windows" ]]; then
            ...
            for extend_app in "${extend_app_names[@]}"; do
                ...
                if [ ! -d "./${product_name}_${extend_app}" ]; then
                    echo "warning: extend app dir [./${product_name}_${extend_app}] No such，skeep zip package"
                    continue
                fi
                 7z a ./${product_name}_${extend_app}_${OS_TYPE_LOWER}_${RUN_MODE}_${CURRENT_VERSION}.zip ./${product_name}_${extend_app} >/dev/null 2>&1
            done
        else
